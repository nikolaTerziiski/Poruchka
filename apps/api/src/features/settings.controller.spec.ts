...
function createPrismaMock() {
  return {
    tenant: {
      update: jest.fn(),
      // Read back by the quiet-hours check, which has to compare a one-sided
      // patch against what is already stored.
      findUnique: jest.fn().mockResolvedValue({ quietHoursStart: 22, quietHoursEnd: 7 }),
    },
  };
}
...
describe("SettingsController tenant settings", () => {
  ...
  it("rejects a quiet-hours window whose start equals its end", async () => {
    const prisma = createPrismaMock();
    const controller = controllerFor(prisma);

    await expect(
      controller.update(tenantId, { quietHoursStart: 9, quietHoursEnd: 9 }),
    ).rejects.toThrow(BadRequestException);
    expect(prisma.tenant.update).not.toHaveBeenCalled();
  });

  it("rejects a one-sided patch that collides with the stored hour", async () => {
    const prisma = createPrismaMock();
    // Stored window is 22 → 7; moving only the start onto 7 would silently
    // disable quiet hours altogether.
    const controller = controllerFor(prisma);

    await expect(controller.update(tenantId, { quietHoursStart: 7 })).rejects.toThrow(
      BadRequestException,
    );
    expect(prisma.tenant.update).not.toHaveBeenCalled();
  });

  it("leaves the tenant unread when the patch touches no quiet-hours field", async () => {
    const prisma = createPrismaMock();
    prisma.tenant.update.mockResolvedValue({ id: tenantId, maxNudges: 4 });
    const controller = controllerFor(prisma);

    await controller.update(tenantId, { maxNudges: 4 });
