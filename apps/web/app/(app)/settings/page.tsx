...
const M = {
  en: {
    title: "Settings",
    subtitle: "Restaurant, notification behavior, and your account",
    ...
    restaurantDesc: "Shown in the sidebar and used for scheduling.",
    ...
    quietOff: "Quiet hours are off — reminders can be sent at any hour.",
    ...
    maxNudgesLabel: "Nudges before escalation",
    maxNudgesHint: "After this many unanswered reminders, the fallback person is notified.",
    minutes: (m: number) => `${m} min`,
    hours: (h: number) => (h === 1 ? "1 hour" : `${h} hours`),
    nudges: (n: number) => (n === 1 ? "1 nudge" : `${n} nudges`),
    // Account
  ...
  bg: {
    title: "Настройки",
    subtitle: "Ресторант, известия и вашият акаунт",
    staffHint: "Само собственици и мениджъри могат да променят тези настройки.",
    saved: "Запазено",
    loadFailed: "Зареждането на настройките не бе успешно.",
    saveFailed: "Записът не бе успешен.",
    restaurantTitle: "Ресторант",
    restaurantDesc: "Показва се в страничната лента и определя графика.",
    ...
    quietOff: "Тихите часове са изключени — напомняния могат да се изпращат по всяко време.",
    ...
    maxNudgesLabel: "Напомняния преди ескалация",
    maxNudgesHint: "След толкова напомняния без отговор се уведомява резервният човек.",
...
function SavedFlash({ show, label }: { show: boolean; label: string }) {
  if (!show) return null;
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 13, color: "var(--status-confirmed-fg)" }}>
      <Check size={14} /> {label}
    </span>
  );
}
...
function InlineError({ text }: { text: string | null }) {
  if (!text) return null;
  return <span style={{ fontSize: 13, color: "var(--red-600)" }}>{text}</span>;
}
...
export default function SettingsPage() {
  ...
  async function saveRestaurant() {
    ...
    try {
      ...
      setRError(e instanceof Error ? e.message : t.saveFailed);
    } finally {
      setRBusy(false);
    }
  }
  ...
  async function saveNotifications() {
    ...
    try {
      ...
      setNError(e instanceof Error ? e.message : t.saveFailed);
    } finally {
      setNBusy(false);
    }
  }
  ...
  async function saveName() {
    ...
    try {
      ...
      setMeError(e instanceof Error ? e.message : t.saveFailed);
    } finally {
      setMeBusy(false);
    }
  }
  ...
  async function changeEmail() {
    ...
    try {
      ...
      setEmailError(e instanceof Error ? e.message : t.saveFailed);
    } finally {
      setEmailBusy(false);
    }
  }
  ...
  async function changePassword() {
    ...
    try {
      ...
      setPwError(e instanceof Error ? e.message : t.saveFailed);
    } finally {
      setPwBusy(false);
    }
  }
        ...
        <>
          ...
          <SectionCard
            ...
            footer={
              canEdit ? (
                <>
                  <Button variant="primary" size="md" onClick={saveNotifications} disabled={nBusy}>
          ...
          >
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(230px, 1fr))", gap: 14 }}>
              ...
              <Field label={t.quietEndLabel} htmlFor="set-quiet-end">
                <Select
                  id="set-quiet-end"
                  value={String(nEnd)}
      ...
      <SectionCard title={t.accountTitle} description={t.accountDesc}>
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ borderTop: "1px solid var(--border-subtle)", paddingTop: 16, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
            <Button variant="ghost" size="md" icon={<LogOut size={15} />} onClick={signOut}>
              {t.signOut}
            </Button>
            {!isSupabaseConfigured ? <span style={{ fontSize: 12, color: "var(--text-faint)" }}>{t.notConfigured}</span> : null}
          </div>
        </div>
      </SectionCard>
    </div>
