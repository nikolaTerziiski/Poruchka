...
function recurrenceSentence(mode: Mode, weekdays: number[], everyNDays: number, time: string, lang: Lang): string {
  ...
  const days = sorted.map((day) => WEEKDAY_LABELS[lang][day - 1]).join(", ");
...
const M = {
  en: {
    ...
    wizardItemsHint: "Click the items this reminder should ask the team to check.",
    ...
    selectedChecklist: "selected - these become the reminder checklist",
    noItemsForSupplier: "No items for this supplier yet.",
    previewEmpty: "Tap items to build the checklist...",
    done: "Done",
    usual: "usual",
  ...
  bg: {
    ...
    wizardItemsHint: "Кликнете артикулите, за които човекът трябва да провери преди поръчка.",
    wizardTimeTitle: "Изберете час",
    wizardTimeHint: "Кой получава напомнянето, колко често се повтаря и в колко часа.",
    stepSupplier: "Стъпка 1 - Доставчик",
    stepItems: "Стъпка 2 - Списък от",
    selectedChecklist: "избрани - стават списъкът за напомняне",
...
function OrderPlanDialog({
  ...
  return (
    <Dialog
      ...
      confirmLabel={!isFinalStep ? t.next : rule ? t.saveChanges : t.finish}
      cancelLabel={c.cancel}
...
function ItemToggle({
  ...
  return (
    ...
    >
      ...
      >
        <Check size={13} strokeWidth={3.5} color="#fff" style={{ opacity: selected ? 1 : 0, transition: "opacity var(--dur-fast) var(--ease-out)" }} />
      </span>
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 14 }}>
          {lines.length ? (
            ...
            <div style={{ fontSize: 13, color: "var(--text-faint)" }}>{labels.empty}</div>
          )}
        </div>
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 14 }}>
      ...
      <div style={{ display: "inline-flex", alignItems: "center", border: "1px solid var(--border-default)", borderRadius: "var(--radius-md)", overflow: "hidden", background: "var(--surface-card)" }}>
        <button type="button" onClick={() => onChange(everyNDays - 1)} style={smallStepperButtonStyle}>-</button>
...
function TimeStepper({
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
      ...
      />
      <span style={{ fontFamily: "var(--font-mono)", fontSize: 38, fontWeight: 700, color: "var(--text-faint)", paddingTop: 28 }}>:</span>
      <TimeColumn
        value={pad2(minute)}
        onUp={() => onMinuteStep(5)}
